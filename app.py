        ...
        self.models = {
            "Free Models": {
                "Llama 2 (7B) - Local": {
                    ...
                    "description": "Open source model - no API key needed"
                },
    ...
    with st.sidebar:
        ...
        if selected_model_config["api_key_required"]:
            api_key = st.text_input(
                "🔑 API Key Required",
                type="password",
                help="This model requires an API key. Get one from the provider's website.",
                placeholder="Enter your API key here..."
            )
            
            # Show helpful info about getting API keys
            if not api_key:
                st.info(" **Free Tier Models**: Even 'free' models require API keys for authentication. Get them from:")
                if "gemini" in selected_model_config["type"]:
                    st.markdown("- [Google AI Studio](https://makersuite.google.com/app/apikey)")
                elif "openai" in selected_model_config["type"]:
                    st.markdown("- [OpenAI Platform](https://platform.openai.com/api-keys)")
                elif "anthropic" in selected_model_config["type"]:
                    st.markdown("- [Anthropic Console](https://console.anthropic.com/)")
        else:
            api_key = None
            st.success("✅ **No API Key Required** - This model works without authentication!")
